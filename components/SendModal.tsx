      ...
      if (token === "SOL") {
        ...
        try {
          ...
          tx.add(createAssociatedTokenAccountInstruction(
            fromPubkey, recipientATA, toPubkey, USDC_MINT
          ));
        }
        tx.add(createTransferInstruction(
          senderATA, recipientATA, fromPubkey,
          ...
          <>
            <div className="flex gap-2 mb-5">
              {(["SOL","USDC"] as const).map((t) => (
                <button key={t} onClick={() => setToken(t)}
                  className={`flex-1 py-2 rounded-xl text-sm font-medium border transition ${token === t ? "bg-accent text-black border-accent" : "border-line text-muted hover:border-accent"}`}>
                  {t}
                </button>
              ))}
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={to} onChange={(e) => setTo(e.target.value)} placeholder="Solana address..."
              ...
              <div>
                ...
                <input value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" type="number" step={token === "SOL" ? "0.001" : "0.01"}
                  className="mt-1 w-full rounded-xl bg-black border border-line px-3 py-2 text-sm font-mono focus:outline-none focus:border-accent" />
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSend} disabled={status === "loading" || !to || !amount}
